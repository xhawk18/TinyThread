/******************************************************************************
 * @file     SDCard.c
 * @brief    SD Card library source code
 * @version  1.0.0
...
#define BACK_FROM_ERROR { SingleWrite(0xFF);SPI_ClrSS(SPI1, SPI_SS0); return FALSE;}
...
uint32_t SDCARD_Open(void)
{
    ...
    /* Set SPI1 pin, GPB0->MOSI0, GPB1->MISO0; GPB2->CLK, GPB3->SS0 */
    GCR->PB_L_MFP = GCR->PB_L_MFP & ~0x0000ffff|0x00003333;
    
    //select SPI1 clock source is from PLL
	...
	SPI_SetMSBEndian(SPI1);
	SPI_SetTxRegister(SPI1,Temp,2);
	
	SPI_SetClockFreq(SPI1,300000,0);
	DBG_PRINTF("Now, SPI is running at %d Hz\n", SPI_GetClock1Freq(SPI1));	
	...
	if (Is_Initialized)
	{
		printf/*DBG_PRINTF*/("SDCARD INIT OK\n\n");		
	}
	else
	{
	 	printf/*DBG_PRINTF*/("SDCARD INIT FAIL\n\n");
		return 1;
	}
		
	SPI_SetClockFreq(SPI1,24000000,0);
	DBG_PRINTF("Now, SPI is running at %d Hz\n", SPI_GetClock1Freq(SPI1));
