...
#define MAX_PACKET_SIZE_INT_IN		8
...
#define HID_KEYBOARD_REPORT_DESCRIPTOR_SIZE \
    sizeof(HID_KeyboardReportDescriptor) / sizeof(HID_KeyboardReportDescriptor[0])
...
const uint8_t USB_ConfigDescriptor[] =
{
	...
	EP_INT,			/* bmAttributes */
	/* wMaxPacketSize */
	MAX_PACKET_SIZE_INT_IN & 0x00FF,
	(MAX_PACKET_SIZE_INT_IN & 0xFF00) >> 8,
...
const uint8_t USB_ProductStringDesc[] = {
	...
	'&', 0, 'H', 0, 'I', 0, 'D', 0, ' ', 0, 'D', 0, 'e', 0, 'v', 0, 'i', 0,
	'c', 0, 'e', 0
};
...
uint32_t USB_ProcessDescriptor(uint8_t *pCtrl, uint32_t CtrlMaxPacketSize)
{
	...
	switch (gUsbCtrl.UsbSetupBuf[3])
	{
		...
		case DESC_HID_RPT:
		{
			...
			if (gUsbCtrl.ReqLen >= CtrlMaxPacketSize)
			{
				my_memcpy(pCtrl, (void *)HID_KeyboardReportDescriptor, CtrlMaxPacketSize);
				gUsbCtrl.pDesc = (uint8_t *)&HID_KeyboardReportDescriptor[CtrlMaxPacketSize];
				gUsbCtrl.EP0_Toggle = 1;
				USBD->MXPLD0 = CtrlMaxPacketSize;
				gUsbCtrl.ReqLen -= CtrlMaxPacketSize;
				gUsbCtrl.StateFlag = FLAG_EP0_DATA_IN;
			...
			{
				my_memcpy(pCtrl, (void *)HID_KeyboardReportDescriptor, gUsbCtrl.ReqLen);
				USBD->MXPLD0 = gUsbCtrl.ReqLen;
				gUsbCtrl.StateFlag = FLAG_OUT_ACK;
			}
			break;
...
{
	/* Interrupt IN transfer ACK */
	uint8_t *buf;

	buf = (uint8_t *)((uint32_t)USBD_SRAM_BASE + (uint32_t)USBD->BUFSEG4);
...
{
	/* start to IN data (Interrpt pipe) */
	USB_EpAck2();
}
...
int32_t USB_MainProcess(void)
{
	...
	sParam.UsbDeviceClass = (USB_CLASS_AUDIO | USB_CLASS_HID);
	...
	sParam.EP[2].u32MaxPacketSize = MAX_PACKET_SIZE_INT_IN;
