/******************************************************************************
 * @file     Smpl_Main.c
 * @brief    Nano1xx NUTINY-EVB_NANO130 PDRDWN_LCD_RTC_DEMO Demo code
 *           System enters power down mode while keeps LCD display. And system
 *           will wake up for every one minute due to RTC alarm interrupt
 * @version  1.0.1
...
void Enter_PowerDown()
{
  ...
  GCR->PA_L_MFP &= 0x0000FFFF; /* PA.4 ~ PA.7 */
  GCR->PA_H_MFP &= 0xFFFF0000; /* PA.8 ~ PA.11 */
  GCR->PB_L_MFP = 0;           /* PB.0 ~ PB.7 */
  GCR->PB_H_MFP &= 0x0000FFF0; /* PB.15 ~ PB.12, PB.8 */
  GCR->PC_L_MFP &= 0xFF0000FF; /* PC.5 ~ PC.2 */
  GCR->PC_H_MFP &= 0x00FFFFFF; /* PC.15 ~ PC.14 */
  GCR->PD_L_MFP &= 0x0000FFFF; /* PD.7 ~ PD.4 */
  GCR->PD_H_MFP = 0;           /* PD.15 ~ PD.8 */
  GCR->PE_L_MFP &= 0x0FFFFFFF; /* PE.7 */
  GCR->PE_H_MFP &= 0x000FFFF0; /* PE.15 ~ PE.13, PE8 */
#endif
