...
void SARM_BS616LV4017(void);

uint32_t u32DataArrary[4] = {0x00000000, 0xFFFFFFFF, 0x5A5A5A5A, 0xA5A5A5A5};


/**
  * @brief  Compare data is matched or not. If compare failed, program will panic in place.
  * @param  u32Param0: Index to the test patter array <u32DataArrary>
  *         u32Param1: Toatl length in bytes to be compared
  *         u32Param2: Offset value added to test data
  *         u32Param3: 0: byte; 1: half-word; 2: word access
  * @retval None
  */
void SARM_CompareFunction(uint32_t u32Param0, uint32_t u32Param1, 
							uint32_t u32Param2, uint32_t u32Param3)
{
	uint8_t 	*pu8DevPtr;
	uint16_t 	*pu16DevPtr;
	uint32_t 	*pu32DevPtr;
...
/**
  * @brief  External SRAM on EBI bus access test
  *         Data Width : 16bits, Data Size : 128K
  *         Byte, half-word, and word access test.
  * @param  None
  * @retval None
  */
void SARM_BS616LV4017(void)
{
	...
	uint32_t 	u32EBIsize;
	uint8_t 	u8TestCondition = 0x0;

	if (u8TestCondition == 0x0)
	{
		u8TestCondition = 0x1;
	...
	else if (u8TestCondition == 0x1)
	{
		u8TestCondition = 0x0;;
		// Enable EBI function with 8-bit data width
 		EBI_Init(1);
		u32EBIsize = EBI_MAX_SIZE / 2;
		printf("   >> Data Width use 8bits and Data Size is %dKB << \n", (u32EBIsize/1024));
	}
	else if (u8TestCondition == 0xFF)
	{
		printf("   >> Exit SARM_BS616LV4017 test << \n\n");
		return ;
	}
	...
	while (u32DataIdx < 5)
	{
		if (u32DataIdx < 4)
		{
			...
			while (u32Idx < u32EBIsize)					
			{
				//printf("pu8DevPtr = 0x%x\n", (int)pu8DevPtr);
				*pu8DevPtr++ = (uint8_t)(u32WriteData);
				u32Idx = u32Idx + 1;   
			}
			// Read from SARM and compare data
			SARM_CompareFunction(u32DataIdx, u32EBIsize, 0, 0);	// param3:0, Byte Write
			printf("PASS\n");
		}else
		{
			// Unfixed data pattern
			...
			SARM_CompareFunction(u32DataIdx, u32EBIsize, (uint32_t)&u32Idx, 0);	// param3:0, Byte Write
			printf("PASS\n\n");	
		}
		u32DataIdx++;
	}
	...
	while (u32DataIdx < 5)
	{
		if (u32DataIdx < 4)
		{
			...
			// Read from SARM and compare data
			SARM_CompareFunction(u32DataIdx, u32EBIsize, 0, 1);	// param3:1, Half Word Write
			printf("PASS\n");
		}else
		{
			// Unfixed data pattern
			...
			SARM_CompareFunction(u32DataIdx, u32EBIsize, (uint32_t)&u32Idx, 1);	// param3:1, Half Word Write
			printf("PASS\n\n");	
		}
		u32DataIdx++;
	}
	...
	while (u32DataIdx < 5)
	{
		if (u32DataIdx < 4)
		{
			...
			// Read from SARM and compare data
			SARM_CompareFunction(u32DataIdx, u32EBIsize, 0, 2);	// param3:2, Word Write
			printf("PASS\n");
		}else
		{
			// Unfixed data pattern
			...
			SARM_CompareFunction(u32DataIdx, u32EBIsize, (uint32_t)&u32Idx, 2);	// param3:2, Word Write
			printf("PASS\n\n");	
		}
		u32DataIdx++;
	}
