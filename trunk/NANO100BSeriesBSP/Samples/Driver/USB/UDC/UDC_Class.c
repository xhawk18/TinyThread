		...
		switch (gMassInfo.sCBW.u8OPCode)
		{
			...
			case UFI_READ_10:
			{
				...
				if(gMassInfo.Length == 0)
				{
					...
					/* Get LBA address */
					gMassInfo.Address = get_be32(&gMassInfo.sCBW.au8Data[0]);
					gMassInfo.LbaAddress = gMassInfo.Address * UDC_SECTOR_SIZE;
					gMassInfo.Length = gMassInfo.sCBW.dCBWDataTransferLength;
					...
					if(gMassInfo.sCBW.bCBWLUN == gMassInfo.F_DATA_FLASH_LUN)
					{
						...
						DataFlashRead(gMassInfo.Address * UDC_SECTOR_SIZE, i, (unsigned int)STORAGE_DATA_BUF);
					    gMassInfo.BytesInStorageBuf = i;
					    gMassInfo.LbaAddress += i;
					}
				}
			...
			case UFI_WRITE_10:
			{
				if (gMassInfo.Length == 0)
				{
					gMassInfo.Length = gMassInfo.sCBW.dCBWDataTransferLength;
					gMassInfo.Address = STORAGE_DATA_BUF;
					gMassInfo.DataFlashStartAddr = get_be32(&gMassInfo.sCBW.au8Data[0]) * UDC_SECTOR_SIZE;
				}
...
void UsbRead(void)
{
    ...
    if (gMassInfo.Length)
    {
    	...
    	else
    	{
			...
			DataFlashRead(gMassInfo.LbaAddress, u32Len, (uint32_t)STORAGE_DATA_BUF);
		    gMassInfo.BytesInStorageBuf = u32Len;
		    gMassInfo.LbaAddress += u32Len;
            gMassInfo.Address = STORAGE_DATA_BUF;		    
...
void UsbWrite(void)
{
	...
	if (gMassInfo.Length > MAX_PACKET_SIZE_BULK)
	{
		...
		{
			if (gMassInfo.sCBW.bCBWLUN == gMassInfo.F_DATA_FLASH_LUN)
				DataFlashWrite(gMassInfo.DataFlashStartAddr, STORAGE_BUFFER_SIZE, (uint32_t)STORAGE_DATA_BUF);
		    
		    gMassInfo.Address = STORAGE_DATA_BUF;
		    gMassInfo.DataFlashStartAddr += STORAGE_BUFFER_SIZE;
		}
	}
	else
	{
		...
		if (gMassInfo.sCBW.u8OPCode == UFI_WRITE_10)
		{
			...
			if (gMassInfo.sCBW.bCBWLUN == gMassInfo.F_DATA_FLASH_LUN)
			{
			    len = lba * UDC_SECTOR_SIZE + gMassInfo.sCBW.dCBWDataTransferLength - gMassInfo.DataFlashStartAddr;
			    if (len)
			    {
				    DataFlashWrite(gMassInfo.DataFlashStartAddr, len, (uint32_t)STORAGE_DATA_BUF);
				}
		    }
		}
...
uint8_t Flash_Identify(uint8_t tLUN)
{
    ...
    {
        gMassInfo.dataFlashTotalSectors = DATA_FLASH_STORAGE_SIZE / UDC_SECTOR_SIZE;
