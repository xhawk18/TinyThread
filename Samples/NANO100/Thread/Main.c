...
static unsigned int	stack[THREAD_NUM][TT_THREAD_BUFFER_SIZE (164) / sizeof (unsigned int)];
...
#define TT_SYSTICK_CLOCK		SysGet_HCLKFreq()
	...
	GPIO_SetBit(LED_GPIO_RED_GRP, LED_GPIO_RED_BIT);		/* OFF */
	GPIO_SetBit(LED_GPIO_ISP_GRP, LED_GPIO_ISP_BIT);		/* OFF */
	GPIO_SetBit(LED_GPIO_ISP_GRP, LED_GPIO_ISP_BIT);		/* OFF */

	GPIO_ClrBit(LED_GPIO_GREEN_GRP, LED_GPIO_GREEN_BIT);	/* ON */
